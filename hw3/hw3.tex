...
In each case the spikes occur at +f and -f. In this example the sampling frequency is 100Hz, therefore the Nyquist frequency is 50Hz. In the images below, as the signal frequency approaches the Nyquist frequency, more aliasing occurs.
...
\begin{figure}[H]
	...
	\caption{\( x(t) = cos(2\pi f_1t) + cos(2\pi f_2t)\) for $f_1$=100 and $f_2$=150Hz}
\end{figure}
\pagebreak
...
\begin{figure}[H]
	...
	\caption{\(x(t) = cos(2\pi f1t) + cos(2\pi f2t)\) for $f_1$ = 100 and $f_2$ = 125 Hz}
\end{figure}
\pagebreak
...
\begin{enumerate}[label=\alph*.]
	...
	\item Magnitude spectrums plotted below
	\begin{figure}[H]
		\centering
		\begin{subfigure}{.5\textwidth}
			\centering
			\includegraphics[width=.9\textwidth]{p3_25bx}
			\caption{magnitude spectrum of x}
			\label{fig:sub1}
		\end{subfigure}%
		\begin{subfigure}{.5\textwidth}
			\centering
			\includegraphics[width=.9\textwidth]{p3_25by}
			\caption{magnitude spectrum of output}
			\label{fig:sub2}
		\end{subfigure}
		\label{fig:test}
	\end{figure}
	\item 71\% of the outputs are 1 and the other 29\% of outputs are 3
		...
		\begin{subfigure}{.5\textwidth}
			\centering
			\includegraphics[width=.9\textwidth]{p3_25cx}
			\caption{magnitude spectrum of x}
			\label{fig:sub1}
		\end{subfigure}%
		\begin{subfigure}{.5\textwidth}
			\centering
			\includegraphics[width=.9\textwidth]{p3_25cy}
			\caption{magnitude spectrum of output}
			\label{fig:sub2}
		\end{subfigure}
		\label{fig:test}
	\end{figure}
	\item randn(1,n) generates a 1-by-n vector of random numbers.
\end{enumerate}
\pagebreak
...
Using the definition of a Fourier transform:\\
 \( F(x(t)) = \int_{-\infty}^{+\infty} x(t)e^{-j2\pi t} \,dt \)\\
 \( F(\delta(t - t_0)) = \int_{-\infty}^{+\infty} \delta(t - t_0)e^{-j2\pi t} \,dt \)\\
  \( F(\delta(t - t_0)) = e^{-j2\pi t_0}\)\\
 Using the time-shift property:\\
 \( F(w(t - t_0) = W(f)e^{-j2\pi ft_0}\)\\
 \( W(\delta(t)) = 1\)\\
 \( F(\delta(t - t_0) = e^{-j2\pi ft_0}\)
...
\begin{figure}[H]
	...
	\caption{frequency response of system with sinc impulse response}
\end{figure}
This figure represents a narrow band pass filter. For this image the spectrum peaks are located at the frequency of the sine wave.\\
\(25 = 2*\pi*3.979\) so \(f = 4Hz\) which corresponds to the spectrum peaks in the image above.
\pagebreak

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Plotting by hand --- DONE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
