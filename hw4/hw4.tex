...
\begin{figure}[H]
	...
	\caption{Figure 5.9}
\end{figure}
Consider the transmission system of Figure 5.9. The message signal $w(t)$ has the magnitude spectrum shown in part (a). The transmitter in part (b) produces the transmitted signal x(t), which passes through the channel in part (c). The channel scales the signal and adds narrowband interferers to create the received signal r(t). The transmitter and channel parameters are φ1 = 0.3 radians, f1 = 24.1 kHz, f2 = 23.9 kHz, f3 = 27.5 kHz, f4 = 29.3 kHz, and f5 = 22.6 kHz. The receiver processing r(t) is shown in Figure 5.9(d). All bandpass and lowpass filters are considered ideal, with a gain of unity in the passband and zero in the stopband.
\begin{enumerate}[label=\alph*.]
...
\begin{enumerate}[label=\alph*.]
    ...
    \item \(x_5(t) = x_4(t)\cos(2\pi f_5t + \phi)\)\\
		...
		min frequency = 150MHz and max frequency = 160MHz\\
		absolute bandwidth = 10MHz
    \item \(x_6(t) = x_5(t)\) when < $f_6$ and 0 otherwise
    \item The max frequency of $x_3(t)$ is 170MHz
    \item The max frequency of $x_5(t)$ is ????
\end{enumerate}
\pagebreak
...
The output of the filter is multiplied by the constant $\pi/2$ FINISH EXPLANATION\\
\(envv=(\pi/2)*filter(b,1,abs(v))\)
\begin{figure}[H]
	\centering
...
I was able to obtain these graphs
   ...
   \end{figure}
	...
	\caption{the demodulated signals (before the LPF) $x_1$ and $x_2$}
   \end{figure}    
    \item
    \begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{q6c} 
	\caption{the recovered signals (after the LPF) $m_1$ and $m_2$}
   \end{figure}        
\end{enumerate}
\pagebreak
